    ...
    <section id="features" className="mt-16">
      ...
      <div className="mt-8 grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        {items.map((it, idx) => (
          <div 
            key={it.id} 
            className="p-6 rounded-2xl bg-gradient-to-br from-white/5 to-white/2 border border-white/10 hover:border-cyan-400/50 hover:from-white/8 transition transform hover:translate-y-[-4px] hover:shadow-lg hover:shadow-cyan-400/20"
          >
            ...
            <div className="mt-3 text-sm opacity-85 leading-relaxed">{it.desc}</div>
          </div>
        ))}
      </div>
    </section>
